...
export default function Rightbar({ user }) {
  ...
  const getPhotos = async () => {
    try {
      ...
      console.log("🚀 ~ file: Rightbar.jsx ~ line 58 ~ getPhotos ~ res", res)

      const items = await res.map((photo) => {
        return {
          original: photo.url,
  ...
  const getUsers = async () => {
    try {
      ...
      const response = await fetch('https://jsonplaceholder.typicode.com/users?_start=0&_limit=10');
      const res = await response.json();
      console.log("🚀 ~ file: Rightbar.jsx ~ line 80 ~ getUsers ~ res", res)
